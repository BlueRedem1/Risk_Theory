...
&= 2\theta^{2} \left[\frac{\theta - 2u}{2u^{2}}|_{\theta}^{\beta + \theta}\right] + \beta 2\theta^{2}\left( \frac{1}{2(\beta + \theta)^{2}}\right)\\
...
\begin{align*}
&= \sum_{K = n * m}^{\infty}t^{K}g_{K} + \sum_{k = 0}^{(n * m)-1}t_{K}g_{K}\\
&= \sum_{K = n * m}^{\infty}t^{K}g_{K} + \sum_{k = 0}^{(n * m)-1}t_{K}*0\\
...
$$ P_{S}(t) =[P_{X}(t)]^{n}$$
Derivando respecto de t, tenempos que:
...
$$\left[mt^{m}f_{m} + t_{(m+1)}^{m + 1}f_{m+1} + t_{m+2}^{m + 2}f_{m + 2}(m+2)\right] \left[t^{nm}g_{nm} + t^{nm+1}g_{nm+1} + t^{nm+2}g_{nm+2}\right]$$
...
rametrizando, obtenemos: $\forall x \geq nm + m$ 
...
\implies [f_{m}(x-m-nm)]g_{x-m} &= (n+1)\sum_{j=m+1}^{x-nm}jf_{j}g{x-j} - \sum_{j=m+1}^{x-nm}(x)f_{j}g_{x-j}\\
...
\genfrac{(}{)}{0pt}{}{500}{i}0.15^{i}\quad 0.85^{500-i} = 500,000 \dots \textcolor{green}{(2)}
\end{align*}
