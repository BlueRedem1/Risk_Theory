...
# todo es el argumento que nos dice si queremos imprimir todas las probas o solo la de x
...
```{r}
sum(PrilII_General(x=1650,n=n,m=m,f=f,todo=T))
```
    
    \item $\mathbb{P}[S\geq1,560]$
```{r}
sum(PrilII_General(x=1560,n=n,m=m,f=f,todo=T))
```
